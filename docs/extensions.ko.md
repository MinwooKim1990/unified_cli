...
확장은 `unified-cli` 0.5.2 배포판에 Core와 함께 들어 있습니다. 하나의 wheel에
...
검증한 읽기 도구 제한 **Preview**입니다. 나머지 provider도 공통 transport를 fixture로
검증한 뒤 명시적으로 선택하면 실행 가능한 **Preview**입니다. vendor CLI·계정 호환성은
보장하지 않습니다. Core 서버 모드에서 활성화된 Ext provider는 없습니다.
...
명시적인 provider 요청이 있을 때만 해당 엔트리포인트 하나를 로드합니다. 18개 항목은
모두 실행 가능한 Preview adapter이며 탐색 자체는 import 없이 수행됩니다. Grok은 명시적으로
선택한 로컬 바이너리가 정확한 `0.2.111` 버전과 제한된 기능 probe를 통과해야만 계속
실행됩니다. 모든 Ext provider의 서버 모드는 계속 비활성입니다.
...
18개 카탈로그 항목은 모두 명시적으로 선택하면 실행 가능한 **Preview**입니다. Grok에는
대표 인증 실사용 근거가 있고, 나머지는 공통 transport fixture를 검증했지만 vendor·계정
호환성을 보장하지 않습니다. 모든 Ext 서버 정책은 비활성입니다.
