...
Stage 5B–5F는 정확히 18개의 비활성 Held provider 카탈로그 메타데이터를 설치합니다.
...
Grok, Kimi, Copilot, Cursor 행은 현재 공식 문서 조사와 고정된 향후 lab 목표를 기록할
뿐이며, 캡처한 CLI 출력이나 검증된 지원을 뜻하지 않습니다. 프롬프트는 argv 값이 되므로
로그에 남겨서는 안 됩니다. 각 항목에는 격리된 Stage 6 식별·프레이밍, 권한/MCP,
인증/세션/모델, 취소/정리, 업데이트/제거, 사용량/오류 근거가 여전히 필요합니다. factory는
바이너리 탐색, 환경 읽기, 실행 전에 거부하며 ACP 후보도 활성화된 bridge가 아닙니다.

| Provider ID | 공식 바이너리/패키지 | 후보 전송 방식 | 잠정 어댑터 목표 | 상태 | 자동 업데이트 차단 | 공식 문서 |
|---|---|---|---|---|---|---|
| `grok` | xAI Grok Build (`grok`, `@xai-official/grok`); 관련 없는 `@vibe-kit/grok-cli`는 거부 | 스트리밍 JSONL 후보 | `-p` one-shot 후보; Core capability 없음 | Held | 문서 기반 strict sandbox/권한 필터 후보와 `GROK_DISABLE_AUTOUPDATER=1`; zero-write나 MCP 시작 비활성화를 증명하지 않음 | [저장소](https://github.com/xai-org/grok-build) · [개요](https://docs.x.ai/build/overview) · [CLI reference](https://docs.x.ai/build/cli/reference) · [Headless scripting](https://docs.x.ai/build/cli/headless-scripting) |
