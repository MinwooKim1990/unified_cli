...
확장 소스를 정리한 곳입니다. 계획된 0.5.1 릴리스에서는 독립적으로 build하거나 install할 수
없으며, 하나의 `unified-cli` wheel이 `unified_cli`와 `unified_cli_ext`를 모두 제공합니다.
확장 기능은 전송·런타임 계약(contract), 제한된 Grok Build Preview 어댑터 하나, 실행 가능한
Qoder·Kilo·Poolside Experimental 어댑터 셋, 비활성 Held 카탈로그 14개를 제공합니다.
...
Qoder, Kilo, Poolside는 실행 가능한 **Experimental** `chat` 통합입니다. 나머지 14개
항목은 **Held**이며 Core 플러그인이 실행 capability를 표시하지 않고 provider를 만들거나
명령을 실행할 수 없습니다. 모든 확장 서버 정책은 비활성입니다.
...
위치 인자 프롬프트와 설정 경계를 검증해야 합니다. 이 세 항목과 나머지 카탈로그는 Held이고,
Qoder, Kilo, Poolside는 별도의 실행 가능한 Experimental 통합입니다.

통합 0.5.1 릴리스에는 자격 증명, 로그인 흐름, 유료 서비스 호출이 포함되지 않습니다.
...
`python -m pip install --force-reinstall "unified-cli==0.5.1"`를 실행하세요. 공개 PyPI에는
...
프로토콜 SDK는 계속 선택 사항이며 `unified-cli` 0.5.1의 Core 또는 Grok 사용에는
필요하지 않습니다. Qoder, Kilo, Poolside Experimental 연동을 명시적으로 사용할 때는
`acp` extra가 필요하지만, 이를 설치하는 것만으로 provider가 선택되거나 실행되지는
않습니다. 제공되는 extra는 `acp`, `mcp`, 두 프로토콜 SDK를 함께 설치하는 `all`,
테스트 의존성용 `dev`입니다.
...
이 릴리스는 오프라인 fixture로 검증한 Grok Preview 하나, 실행 가능한 Qoder·Kilo·Poolside
Experimental 통합 셋, Held 항목 14개가 있는 기반 패키지입니다. 대표 인증 native smoke는
통과했지만 폭넓은 provider 호환성을 입증하지 않으므로 Grok은 Stable이 아닙니다. Ext provider는
Core 로컬 서버에 노출되지 않습니다.
